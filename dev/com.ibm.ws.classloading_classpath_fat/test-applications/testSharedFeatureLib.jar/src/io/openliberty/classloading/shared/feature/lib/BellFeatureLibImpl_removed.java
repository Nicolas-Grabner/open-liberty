...
/**
 * Bell library implementation of {@link TestFeatureApi} used by the State 2
 * "removed" success probe of Test 3 (Bell classloader test).
 * <p>
 * This class implements the <em>already-cached</em> {@link TestFeatureApi} interface.
 * Because {@code TestFeatureApi} was successfully resolved via the Bell library
 ...
 * Bell library {@code AppClassLoader} is not recycled on feature removal and that
 * previously-loaded types remain accessible. Compare with {@link BellFeatureLibImpl2},
 * which implements the never-before-seen {@code TestFeatureApi2} and is expected to
 * produce a {@link NoClassDefFoundError}.
 */
public class BellFeatureLibImpl_removed implements TestFeatureApi {

    @Override
    public String doWork() {
        return "BellFeatureLibImpl_removed.doWork() called successfully from Bell library (state 2, cached interface)";
    }
}
